...
После установки запустите базу данных одним из способов:
...
После запуска откроется интерактивная консоль, в которой можно выполнять команды для управления таблицами и данными.
...
**Особенности:**
- **Требует подтверждения** — перед удалением система запросит подтверждение операции
- Операция необратима — все данные таблицы будут безвозвратно удалены
- При отмене операции (ввод любого значения кроме `y`) таблица не будет удалена
...
- Операция необратима
- При отмене операции (ввод любого значения кроме `y`) записи не будут удалены
...
```
Вы уверены, что хотите выполнить "удаление записей из таблицы"? [y/n]: n
Операция отменена
```
...
Система запрашивает подтверждение перед выполнением опасных операций, которые могут привести к потере данных:

- **`drop_table`** — удаление таблицы
- **`delete`** — удаление записей из таблицы

При выполнении этих команд система выведет запрос на подтверждение:
...
**Важно:**
- Для подтверждения операции введите `y` или `Y`
- Любой другой ввод (включая `n`, `N` или пустую строку) отменит операцию
- При отмене операции выводится сообщение "Операция отменена"

### Обработка ошибок

Система автоматически обрабатывает различные типы ошибок и выводит понятные сообщения пользователю:

**Обрабатываемые типы ошибок:**
- `KeyError` — ошибки доступа к несуществующим таблицам или ключам
- `ValueError` — ошибки валидации данных (неправильные типы, форматы)
- `FileNotFoundError` — ошибки доступа к файлам базы данных

**Особенности:**
- При возникновении ошибки выводится понятное сообщение с описанием проблемы
- Операция безопасно завершается без прерывания работы программы
- Пользователь может продолжить работу с другими командами
